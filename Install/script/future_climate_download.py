...
start_date=arcpy.GetParameterAsText(2)
end_date=arcpy.GetParameterAsText(3)
boundary_input=arcpy.GetParameterAsText(4)

os.chdir(path)
CreateFolder('FUTURE_CLIMATE')
outpath=os.path.join(path,'FUTURE_CLIMATE')
os.chdir(outpath)
...
urls='http://dataserver3.nccs.nasa.gov/thredds/dodsC/bypass/NEX-GDDP/bcsd/'+scenario+'/r1i1p1/pr/'+model+'.ncml'
res=0.25

arcpy.gp.addMessage('Trying to Connect....please wait!')
arcpy.gp.addMessage(urls)
...
#arcpy.gp.addMessage(lat[lati])
#arcpy.gp.addMessage(loni)
data_all=pds['pr']
f=open(str(model).upper()+'_'+str(scenario).upper()+'.BIN','wb+')
...
for dt in [st_date + timedelta(xx) for xx in range((en_date - st_date).days + 1)]:
    ...
    if len(ti) > 0:
        ...
        data=data_all['pr'][ti[0],lati[-1]:lati[0]+1,loni[0]:loni[-1]+1]
        #dd=data['pr'][:]
        ddd=data[0,:,:]
        ddd[ddd<0]=0
