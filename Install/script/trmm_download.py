...
os.chdir(path)
CreateFolder('TRMM')
outpath=os.path.join(path,'TRMM')
os.chdir(outpath)
...
urls='http://iridl.ldeo.columbia.edu/SOURCES/.NASA/.GES-DAAC/.TRMM_L3/.TRMM_3B42/.v7/.daily/.precipitation/dods'
res=0.1
...
for dt in [st_date + timedelta(xx) for xx in range((en_date - st_date).days + 1)]:
    ...
    if len(ti) > 0:
        ...
        data=pds['precipitation'][ti[0],lati[-1]:lati[0]+1,loni[0]:loni[-1]+1]
        dd=data['precipitation'][:]
        ddd=dd[0,:,:]
        ddd[ddd<0]=0
