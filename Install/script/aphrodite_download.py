...
for dt in [st_date + timedelta(xx) for xx in range((en_date - st_date).days + 1)]:
    ...
    if len(ti) > 0:
        ...
        mynew_ras=arcpy.NumPyArrayToRaster(ddd.astype(float),arcpy.Point(left_X,bottom_y),x_cell_size=0.25,y_cell_size=0.25)
        ...
        g_data=simple_idw(lonx,latx,ddd.flatten(),xi,yi)
        gg_data=g_data.reshape((cols,rows))
        grid=np.flipud(gg_data)

        np.savetxt('Data'+str(ti[0])+'.txt',grid,delimiter=',')    

        mynew_ras=arcpy.NumPyArrayToRaster(grid.astype(float),arcpy.Point(left_X,bottom_y),x_cell_size=cell_size,y_cell_size=cell_size)
